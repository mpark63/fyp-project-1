...
function drop(ev) {
    ...
    addCourse(semNum.charAt(1), courseInfo.code, courseInfo.title, courseInfo.credits, courseInfo.areas);
...
function getCreditsTotal() {
    let sum = calculateCredits(s1) + calculateCredits(s2) + calculateCredits(s3) + calculateCredits(s4) +
...
function download() {
    ...
    html2canvas(elementHTML, {
        ...
        onrendered: function(canvas) {
            ...
            pdf.addImage(canvasDataURL, 'PNG', 50, 0, (pageWidth * .5), (pageHeight * .5));
...
function required() {
    ...
    for (let i = 0; i < semesters.length; i++) {
        ...
        for (let j = 0; j < sem.length; j++) { // for each course 
            let areas = sem[j].areas; // for area e.g. 'H' or "NS"
            for (let k=0; k < areas.length; k++){ 
                switch (areas.charAt(k)) {
                    case "N" : nCredits += parseInt(sem[j].credits); break;
                    case "Q" : qCredits += parseInt(sem[j].credits); break; 
                    case "H" : hCredits += parseInt(sem[j].credits); break;
                    case "S" : sCredits += parseInt(sem[j].credits); break;
                } 
            }
        }
    }
    ...
    // message to output 
    let message = "Your credits:\n\nBasic Sciences:  " + nCredits.toFixed(1) + " / 8\nMathematics:  " + qCredits.toFixed(1) + " / 16\nHumanities:  " 
        + hCredits.toFixed(1) + " / 9\nSocial Sciences:  " + sCredits.toFixed(1) + " / 9\nTotal:  " + totalCredits + " / 120\n\n"; 
    ...
    if (canGraduate) { 
        ...
        if (totalCredits<120) message += (120-totalCredits) + " more credits overall\n"; 
    }
